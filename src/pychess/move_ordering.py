...
class MoveOrderer:
    ...
    Killer and history tables are per-instance; a fresh ``MoveOrderer`` (built
    per search, or per Lazy SMP worker) starts empty.
    """

    def __init__(self, root_moves: set[chess.Move] | None = None) -> None:
        self.killers: dict[int, list[chess.Move]] = {}
        self.history: dict[tuple[chess.Color, chess.Square, chess.Square], int] = {}
        # Restricts the moves considered at the root; Lazy SMP workers each own
        # a slice of the root moves. ``None`` means "all".
        self.root_moves = root_moves

    # ---- ordering -------------------------------------------------------

    def order_moves(
        self, board: chess.Board, tt_move: chess.Move | None = None, ply: int = 0
    ) -> list[chess.Move]:
        killers = self.killers.get(ply, ())
        history = self.history
        color = board.turn
        root_moves = self.root_moves if ply == 0 else None
    ...
    def mvvlva(self, board: chess.Board, move: chess.Move) -> int:
