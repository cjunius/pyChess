...
# Indexed by ``chess`` piece type (PAWN=1 .. KING=6); slot 0 is an unused pad.
...
def _to_a1_first(table: list[int]) -> list[int]:
    """Flip a rank-8-first table to python-chess square order (a1 = 0)."""
    return [table[(7 - (i // 8)) * 8 + (i % 8)] for i in range(64)]


MG_PST = [
    _to_a1_first(t) for t in (_MG_PAWN, _MG_KNIGHT, _MG_BISHOP, _MG_ROOK, _MG_QUEEN, _MG_KING)
]
EG_PST = [
    _to_a1_first(t) for t in (_EG_PAWN, _EG_KNIGHT, _EG_BISHOP, _EG_ROOK, _EG_QUEEN, _EG_KING)
]


def pesto_terms(board: chess.Board) -> tuple[int, int, int]:
