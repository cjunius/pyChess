...
from .evaluation import EG_PST, EG_VALUE, MG_PST, MG_VALUE, PHASE_INC
...
class EvalBoard(chess.Board):
    ...
    def __init__(self, fen: str | None = chess.STARTING_FEN, *, chess960: bool = False) -> None:
        super().__init__(fen, chess960=chess960)
        self._eval_stack: list[tuple[int, int, int]] = []
        self._recompute_eval()

    # -- accumulator ---------------------------------------------------------

    def _recompute_eval(self) -> None:
        mg = eg = phase = 0
    ...
    def _acc(
        self, piece_type: chess.PieceType, color: chess.Color, square: chess.Square, sign: int
    ) -> None:
        i = piece_type - 1
